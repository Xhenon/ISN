...
def couleur():
    global r,g,b,coul
    degrade=20
    r=hex(max(255,(int("0x"+r,16)+randint(-2*degrade,2*degrade))%1))
    r="0"*(2-len(r[2:]))+r[2:]
...
def triangle(origine,c,o,coul):
    ...
    if o==1:
        ...
        coul='white'
        if randint(0,10)<7:
            coul =["violet","blue","green","yellow","orange","red"][randrange(6)]
        can.create_polygon(x, y, x, y+c, x-math.sqrt(3)/2*c, y+c/2, x, y,
        outline = 'black', width = 1, fill = str(coul))
coul =["violet","blue","green","yellow","orange","red"]
...
def Clic(event):
    ...
    if Oc==1:
        triangle((Xc-math.sqrt(3)/6*c, Yc-0.5*c), c,1,coul[1]) #centre ou on clic
        triangle((Xc-math.sqrt(3)/6*c, Yc-0.5*c), c,2,"white")
        triangle((Xc-math.sqrt(3)/6*c+44, Yc-0.5*c+25), c,2,"white")
        triangle((Xc-math.sqrt(3)/6*c+44, Yc-0.5*c-25), c,2,"white")
        i=i+1
    else:
        triangle((Xc+math.sqrt(3)/6*c, Yc-0.5*c), c,0,"white") #centre ou on clic
        triangle((Xc+math.sqrt(3)/6*c, Yc-0.5*c), c,1,"white")
        triangle((Xc+math.sqrt(3)/6*c-44, Yc-0.5*c+25), c,1,"white")
        triangle((Xc+math.sqrt(3)/6*c-44, Yc-0.5*c-25), c,1,"white")
    can.after(500)



c =50         # taille des triangles
